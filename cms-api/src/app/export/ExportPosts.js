...
class ExportPosts extends Operation {
  ...
  async execute(data) {
    ...
    const res = await this.sequelize.query(`
      ...
      UNION ALL
      SELECT postId, contributors, category, title, content, IFNULL(locationAddress, ''), IFNULL(tagsOriginal, ''), IFNULL(tagsRetained, ''), IFNULL(tagsRemoved, ''), IFNULL(tagsAdded, ''), status, IFNULL(publishedAt, ''), IFNULL(scheduledAt, ''), IFNULL(expiredAt, ''), createdAt, updatedAt
      FROM posts
